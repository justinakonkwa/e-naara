-- =====================================================
-- 🚀 INSTALLATION COMPLÈTE DU SYSTÈME E-COMMERCE (CORRIGÉ)
-- =====================================================

-- Ce script corrige toutes les tables et installe le système complet
-- Il peut être exécuté plusieurs fois sans erreur

-- =====================================================
-- PARTIE 1: CORRECTION DES TABLES EXISTANTES
-- =====================================================
...
DO $$ 
BEGIN
    -- Ajouter la colonne product_id (TEXT pour correspondre à products.id)
...
-- =====================================================
-- PARTIE 2: SYSTÈME DE PORTEFEUILLE
-- =====================================================

-- Étape 5: Créer la table wallets si elle n'existe pas
...
-- Étape 6: Créer la table wallet_transactions si elle n'existe pas
...
-- Étape 7: Créer les index pour le portefeuille
...
-- Étape 8: Créer la fonction de mise à jour automatique
...
-- Étape 9: Créer le trigger de mise à jour (seulement s'il n'existe pas)
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- Étape 10: Activer RLS pour le portefeuille
ALTER TABLE wallets ENABLE ROW LEVEL SECURITY;
ALTER TABLE wallet_transactions ENABLE ROW LEVEL SECURITY;

-- Étape 11: Créer les politiques RLS pour wallets
DO $$ 
BEGIN
...
-- Étape 12: Créer les politiques RLS pour wallet_transactions
DO $$ 
BEGIN
...
-- Étape 13: Créer la fonction pour créer automatiquement un portefeuille
...
-- Étape 14: Créer le trigger pour créer automatiquement un portefeuille
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
...
-- Étape 15: Créer la fonction pour ajouter de l'argent au portefeuille
...
-- Étape 16: Créer le trigger pour ajouter de l'argent au portefeuille
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
...
-- =====================================================
-- PARTIE 3: GESTION AUTOMATIQUE DES QUANTITÉS
-- =====================================================

-- Étape 17: Créer une fonction pour décrémenter la quantité du produit
...
-- Étape 18: Créer le trigger pour décrémenter automatiquement la quantité
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
...
-- Étape 19: Créer une fonction pour gérer les annulations de commande
...
-- Étape 20: Créer le trigger pour restaurer la quantité lors des annulations
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
...
-- Étape 21: Créer une fonction pour vérifier la disponibilité des produits
...
-- Étape 22: Créer le trigger pour vérifier la disponibilité
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
...
-- =====================================================
-- PARTIE 4: INDEX ET CONTRAINTES
-- =====================================================

-- Étape 23: Créer des index pour optimiser les performances
...
-- Étape 24: Ajouter des contraintes pour assurer la cohérence
DO $$ 
BEGIN
    -- Contrainte pour quantity >= 0 dans products
    IF NOT EXISTS (
    ...
    -- Contrainte pour quantity > 0 dans orders
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
    ...
    -- Contrainte pour total_amount >= 0 dans orders
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
    ...
    -- Contrainte pour status valide dans orders
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
...
-- Étape 25: Ajouter les commentaires
