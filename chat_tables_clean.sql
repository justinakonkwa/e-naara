-- Script pour nettoyer et recréer les tables de chat
...
CREATE POLICY "Users can insert notifications" ON chat_notifications
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM chats 
            WHERE chats.id = chat_notifications.chat_id 
            AND (chats.customer_id = auth.uid()::text OR chats.seller_id = auth.uid()::text)
        )
    );
