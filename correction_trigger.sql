-- Script de correction pour le trigger check_product_availability
...
-- 5. Tester avec le produit problématique
DO $$
DECLARE
    product_record RECORD;
...
BEGIN
    ...
    ELSIF NOT product_record.is_available THEN
        RAISE NOTICE '❌ ERREUR: Le produit % n''est pas disponible', product_id;
    ELSIF product_record.quantity < requested_quantity THEN
        RAISE NOTICE '❌ ERREUR: Quantité insuffisante', product_id;
    ELSE
        RAISE NOTICE '✅ SUCCÈS: Le produit % est disponible!', product_id;
    END IF;
END $$;
